...
- Backend writes user PGP keyrings to `/data` → named volume `data`. Don't bind-mount unless you have a reason; the named volume survives `docker compose down`.
...
- Tests: backend `go test ./...` (add `-race` for CI), frontend `npm run test` (Vitest + Testing Library), workspace `npm test` / `npm run typecheck` (shared + mobile). GitHub Actions: `.github/workflows/test.yml` (Go + web on `main`) and `.github/workflows/mobile.yml` (path-filtered `mobile/**` + `shared/**`).
