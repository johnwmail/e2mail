...
export const SieveEditor: React.FC<Props> = ({ accountId, scriptName, initialContent, onSaved }) => {
  ...
  const handleCheck = async () => {
    ...
    try {
      ...
      setCheckResult({ ok: true, msg: t('sieve.syntaxOk') });
    } catch (e: any) {
      setCheckResult({ ok: false, msg: e.message || t('sieve.syntaxBad') });
    } finally {
      setChecking(false);
    }
  };
  ...
  return (
    <div className="flex flex-col h-full gap-3">
      ...
      <div className="flex-1 flex flex-col min-h-[260px]">
        <label className="text-[11px] font-semibold text-slate-500 mb-1">{t('sieve.scriptLabel', { name: scriptName })}</label>
        <textarea
          value={content}
        ...
        />
        <div className="text-[10px] text-slate-400 mt-1">{t('sieve.pigeonhole')}</div>
      </div>
    </div>
  );
};
