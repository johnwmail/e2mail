    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-white dark:bg-slate-900 rounded-2xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-150">
        <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200 dark:border-slate-800">
          ...
          <button onClick={onClose} className="p-1.5 text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition" aria-label={t('common.close')}>
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="flex items-center justify-between px-4 py-2 border-b border-slate-200 dark:border-slate-800 text-[11px] text-slate-400">
          ...
          <button onClick={() => refetch()} className={`flex items-center gap-1 hover:text-slate-600 transition ${isFetching ? 'animate-spin' : ''}`} aria-label={t('common.refresh')}>
            ...
            <div className="space-y-1">
              {orderedTopFolders.map((f, i) => (
                <div key={f.name} className="flex items-center gap-2 bg-slate-50 dark:bg-slate-800/50 rounded-lg px-2 py-1.5">
                  ...
                  <button onClick={() => moveTop(i, -1)} disabled={i === 0} className="p-1 text-slate-400 hover:text-blue-600 rounded disabled:opacity-20 transition" title={t('folderManager.moveUp')} aria-label={t('folderManager.moveUp')}>
                    <ArrowUp className="w-3.5 h-3.5" />
                  </button>
                  <button onClick={() => moveTop(i, 1)} disabled={i === orderedTopFolders.length - 1} className="p-1 text-slate-400 hover:text-blue-600 rounded disabled:opacity-20 transition" title={t('folderManager.moveDown')} aria-label={t('folderManager.moveDown')}>
