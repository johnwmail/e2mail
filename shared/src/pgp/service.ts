...
export const extractTextFromMime = (mimeText: string): string | null => {
  ...
  const extracted: { html: string | null; plain: string | null } = { html: null, plain: null };
  ...
  const processParts = (text: string, b: string) => {
    ...
    for (const rawPart of rawParts) {
      ...
      if (contentType === 'text/html' && !extracted.html) {
        extracted.html = decodeBody(bodyRaw, encoding);
        continue;
      }
      if (contentType === 'text/plain' && !extracted.plain) {
        extracted.plain = decodeBody(bodyRaw, encoding);
        continue;
      }
    }
  };
  ...
  if (extracted.plain) return extracted.plain;
...
export const fileToPublicKeyArmor = async (file: Blob): Promise<string> => {
...
export const fileToPrivateKeyArmor = async (file: Blob): Promise<string> => {
  const buf = await file.arrayBuffer();
  const bytes = new Uint8Array(buf);
  if (isAsciiText(bytes)) {
    return new TextDecoder('utf-8').decode(bytes);
...
export function createPgpService(client: ApiClient) {
  ...
  saveContactKeysBulk: async (
    ...
    try {
      const result = await client.request<{ saved: number; skipped: string[] }>(
        '/pgp/contacts/bulk',
        {
          method: 'POST',
  ...
  importContactKeysFromFile: async (armored: string): Promise<{
    ...
    return await client.request<{ saved: number; skipped: string[]; invalid: number }>(
      '/pgp/contacts/import',
      {
        method: 'POST',
  ...
  syncKeyringToCloud: async (keyPair: PgpKeyPair): Promise<void> => {
    await client.request<void>('/pgp/keyring', {
      method: 'POST',
      body: JSON.stringify({
  ...
  fetchKeyringFromCloud: async (): Promise<PgpKeyPair | null> => {
    try {
      const payload = await client.request<CloudKeyringPayload | null>('/pgp/keyring');
  ...
  getContactKeys: async (): Promise<PgpContactKey[]> => {
    try {
      return await client.request<PgpContactKey[]>('/pgp/contacts');
    } catch (err) {
  ...
  saveContactKey: async (
    ...
    const result = await client.request<{ contact: PgpContactKey }>('/pgp/contacts', {
      method: 'POST',
      body: JSON.stringify({
        email: finalEmail,
        name: name || info.name,
  ...
  removeContactKey: async (email: string): Promise<void> => {
    await client.request<void>(`/pgp/contacts/${encodeURIComponent(email.toLowerCase().trim())}`, {
      method: 'DELETE',
    });
  },
